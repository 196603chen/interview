...
- [内部属性[[Class]]是什么？]()
    内部属性[[Class]]是判断类型的，它不可访问，只能通过Object.prototype.toString.call()调用，
    Object.prototype.toString内部是先获取[[Class]]属性并返回
    Object.prototype.toString.call([1,2,3]) "[object Array]" //后面这个Array是通过获取[[Class]]属性获取的

- [判断类型的函数]
    Object.prototype.toString.call(null).slice(8,-1)

- [slice sbustr substring]的区别
                    slice                       substr                    substring
    支持             数组和字符串                  字符串                      字符串
    参数             开始索引，结束索引          开始索引，截取个数          开始索引，结束索引
    Hello(1,4)          ell                       ello                       ell
    Hello(4,1)          ""/[]                       o                        ell(自动调换)
    Hello(2,-1)         ll                         ""                      He(第一个参数为正，第二个参数为负的情况，从头截取第一个的个数)

- [介绍 instanceof 原理，并手动实现](#介绍-instanceof-原理并手动实现)
    原理是左侧的__proto__链上和y.prototype相等
...
- [强制类型转换和隐形类型装换]
    ...
    1.转换为数值类型：Number(mix)、
                    parseInt(string,radix)（如果参数不是一个字符串，则将其转换为字符串(使用  ToString 抽象操作)）、
                    parseFloat(string)：将字符串转换为浮点数类型的数值。（规则同parseInt）
    2.转换为字符串类型：toString(radix)、toString(radix)方法。除undefined和null之外的所有类型的值都具有toString()方法，其作用是返回对象的字符串表示。
                      String(mix)
    3.转换为布尔类型：Boolean(mix)：undefined、null、false、NaN、''、0、-0 都转为 false。其他所有值都转为 true，包括所有对象。

                    Number(mix)                                                 parseInt(string,radix)                                 
    布尔值              0、1                                                            NaN
    数字值              本身                                                            NaN
    null                0                                                               NaN 
    undefined           NaN                                                             NaN
    字符串           如果字符串中只包含数字，则将其转换为十进制（忽略前导0）      1、忽略字符串前面的空格，直至找到第一个非空字符
                    如果是空字符串，将其转换为0                                2、如果第一个字符不是数字符号或者负号，返回NaN
                    如果字符串中包含非以上格式，则将其转换为NaN                 3、如果第一个字符是数字，则继续解析直至字符串解析完毕
                                                                                或者遇到一个非数字符号为止
                                                                            4、如果上步解析的结果以0开头，则将其当作八进制来解析；
                                                                                如果以x开头，则将其当作十六进制来解析
                                                                            5.如果指定radix参数，则以radix为基数进行解析

    对象            调用对象的valueOf()方法,然后依据前面的规则转换返回的值               NaN
    ...
    隐形类型转换：
        ...
        ？ ： 中的条件判断表达式
        逻辑运算符||（逻辑或） 和&&（逻辑与）左边的操作数（作为条件判断表达式） 逻辑运算符||和&&返回的是他们其中一个操作数的值，而非条件判断的结果

javascript中局部变量才是被存储在栈上，全局变量存在静态区域上，其它都存储在堆上。，在闭包中被闭包引用的变量存放在堆上，不被闭包引用的局部变量存放在栈上


- [new 的实现原理，动手实现一个 new](#new-的实现原理动手实现一个-new)

new 操作符可以帮助我们构建出一个实例，并且绑定上 this，内部执行步骤可大概分为以下几步：
    以var f=new Fun(a,b)为例：
    新生成了一个对象（创建一个带引用的变量）var f={};
    对象连接到构造函数原型上，并绑定 this（给f增加一个属性_proto_指向构建函数的原型，即f._proto_ = Fun.prototype）
    执行构造函数代码（执行Fun.call(f,a,b)，即创建Fun函数的执行上下文，其中this指向f）
    返回新对象（返回f）（如果手动return一个数值，如果是原始数据则不影响，但是如果是对象就会覆盖掉内部创建的对象）
    function Test(name) {
        this.name = name
